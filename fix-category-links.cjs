...
categories.forEach(cat => {
    ...
    // 1. Update the loop to use <a> tag - more robust regex
    // Matches <article ... class="post-item ..."> ... </article>
    const articleRegex = /<article[^>]*?class="post-item[^>]*?"[^>]*?>([\s\S]*?)<\/article>/g;
    
    if (content.includes('<article')) {
        content = content.replace(articleRegex, (match, inner) => {
            // Check if it already has attributes we need
            return `<a 
        ...
        console.log(`Replaced articles in ${cat}.astro`);
    }

    // 2. Ensure Pagination Logic is present in FM
...
const totalPages = Math.ceil(posts.length / postsPerPage);`);
    ...
    // 3. Ensure Pagination UI is present
    ...
    // 4. Ensure Script has pagination variables
    if (!content.includes('let currentPage = 1;')) {
        content = content.replace('let currentSort = \'desc\';', 
`let currentPage = 1;
  const postsPerPage = 100;
  let currentSort = 'desc';`);
        
        // Add Pagination logic to updateDisplay ONLY IF NOT ALREADY THERE
        if (!content.includes('// 0. Pagination State')) {
...
`;
    ...
    let visibleItems = postItems.filter(item => {
      ...
      return isVisible;
            ...
            // Add initial page reset to search/sort listeners
            content = content.replace('currentSearch = (e.target as HTMLInputElement).value;\n    updateDisplay();', 'currentSearch = (e.target as HTMLInputElement).value;\n    currentPage = 1; updateDisplay();');
            content = content.replace('currentFilter = btn.getAttribute(\'data-class\');\n      updateDisplay();', 'currentFilter = btn.getAttribute(\'data-class\');\n      currentPage = 1; updateDisplay();');
